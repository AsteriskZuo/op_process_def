// op_process_def.cpp : 定义 DLL 应用程序的导出函数。
//

#include "op_process_def.h"
#include <TlHelp32.h>
#include <memory>
...
OpProcess* __stdcall GetSingleObject()
{
	return OpProcessPrivate::createObject();
}
void __stdcall DestroySingleObject(OpProcess* object)
{
	OpProcessPrivate::destroyObject(object);
}

void __stdcall DestroyOpObject()
{
	DestroySingleObject(GetSingleObject());
}

bool __stdcall KillProcessFromNameW(const wchar_t* pname, bool isKillTree /*= false*/)
{
	return GetSingleObject()->killProcessFromNameW(pname, isKillTree);
}

bool __stdcall KillProcessFromNameC(const char* pname, bool isKillTree /*= false*/)
{
	return GetSingleObject()->killProcessFromNameA(pname, isKillTree);
}

bool __stdcall FindProcessFromNameC(const char* pname)
{
	return GetSingleObject()->FindProcessFromNameA(pname);
}

bool __stdcall FindProcessFromNameW(const wchar_t* pname)
{
